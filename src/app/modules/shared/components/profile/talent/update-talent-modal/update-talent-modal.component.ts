...
@Component({
	...
	templateUrl: './update-talent-modal.component.html'
})

export class UpdateTalentModalComponent implements OnInit {
	...
	updateForm() {
		...
		this.form.patchValue({
			...
			instrument: talent.talent || '',
			range: talent.range || '',
			primary: talent.primary || '',
			year_started: talent.year_started || '',
			reg1_range: talent.reg1_range || '',
			reg2_range: talent.reg2_range || '',
			reg3_range: talent.reg3_range || '',
			reg4_range: talent.reg4_range || ''
		});

		this.styles.forEach(function (part, index) {

			if ('styles' in talent) {
				if (talent.styles.includes(part.name)) {
					part.isChecked = true;
				}
			}
		...
		this.emphasis.forEach(function (part, index) {

			if ('emphasis' in talent) {
				if (talent.emphasis.includes(part.name)) {
					part.isChecked = true;
				}
			}
